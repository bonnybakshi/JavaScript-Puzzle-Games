...
function startGame(){
    ...
    var shuffleNum = 100;
    // board element
    var cardBoard = document.getElementById("cardBoard2");
    // length of rows 
    ...
    cardBoard.addEventListener("click", function( event ) {  
        ...
        if(event.target != event.currentTarget && event.target.id != "tile16"){
            ...
            for (var row = 0; row < rowLength; row++) {
                ...
                for (var col = 0; col < colLength; col++){ 
                    if(cardBoard.children[row].children[col].id == selectedTile ){
                        btnRow = row;
                        btnCol = col;
                    }    
                }
            ...
            if (shiftingisPossible(btnRow, btnCol)){
                ...
                selected.setAttribute("id", "tile16");
    
                var blankswap = cardBoard.children[blankRow].children[blankCol];
                blankswap.setAttribute("id", selectedTile);        
            }     
        ...
        for (var row = 0; row < rowLength; row++){
            ...
            for(var col = 0; col < colLength; col++){
                if(row == rowLength - 1 && col == colLength -1){
                    cardBoard.children[row].children[col].id = "tile16";
                } else{
                    cardBoard.children[row].children[col].id = "tile" + tilenum;
                    tilenum++;
                }      
            }  
        }
        ...
        for (var row = 0; row < rowLength ; row++) {
            ...
            for (var col = 0; col < 4; col++){ 
                if(!(row == 3 && col == 3)){
                    ...
                    if(cardBoard.children[row].children[col].id != "tile" + num){
                        return false;
                    } else{
                        num++;
                    }
    ...
    function getBlankRowColIndex(){
        for (var row = 0; row < rowLength; row++){
            ...
            for(var col = 0; col < colLength; col++){
                ...
                if(cardBoard.children[row].children[col].id == "tile16"){
                    blankRow = row;
                    blankCol = col;
    ...
    function shuffle(){
        ...
        for(var i = 0; i <= shuffleNum ; i++) 
        {
            ...
            // iterate over all tiles to find the possible move
            for (var row = 0; row < rowLength; row++) {
                var colLength = cardBoard.children[row].children.length;
                for(var col = 0; col < colLength; col++)
                {
                    // get row and column for blank button 
                    getBlankRowColIndex();
            ...
            // swap the selected tile with blank tile
            var temp = cardBoard.children[listRow[rand]].children[listCol[rand]].getAttribute("id");
            cardBoard.children[listRow[rand]].children[listCol[rand]].setAttribute("id", "tile16");
            cardBoard.children[blankRow].children[blankCol].setAttribute("id", temp);
        } 
    } 
